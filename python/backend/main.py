...
# Mount static directory to serve images, CSS, JS, etc.
...
@app.get("/credit")
async def get_credit_page(request: Request):
    """
    Render the credit form page.

    Args:
        request (Request): FastAPI Request object.

    Returns:
        TemplateResponse: HTML template response for the credit form page.
    """
    return templates.TemplateResponse("credit.html", {"request": request})
...
async def prediction_page(request: Request):
    ...
    Returns:
        TemplateResponse: HTML template response for the prediction page.
    """
